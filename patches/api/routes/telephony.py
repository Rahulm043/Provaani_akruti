...
async def websocket_ari_endpoint(websocket: WebSocket):
    ...
    # Accept with "media" subprotocol â€” chan_websocket sends
...
async def websocket_endpoint(
    ...
    in the path because carriers strip query strings â€” Twilio documents that
...
async def _handle_telephony_websocket(
    ...
    first (see ``ws_auth``) closes the forgery gap â€” the id triple is no longer
    enough on its own â€” but only once an operator sets a secret.
...
async def handle_inbound_run(request: Request):
    ...
    Provider-specific signature/timestamp headers are not enumerated here â€”
    ...
    try:
        ...
        logger.info(
            f"/inbound/run normalized data â€” provider={normalized_data.provider} "
        ...
        event_name = str(webhook_data.get("Event") or "").strip().lower()
        call_status = str(normalized_data.call_status or webhook_data.get("CallStatus") or "").strip().lower()
        if event_name in ("hangup", "completed", "rejected", "canceled") or call_status in ("completed", "hangup", "canceled", "failed", "busy"):
            logger.info(
                f"/inbound/run: ignoring terminal event='{event_name}' status='{call_status}' for call_id={normalized_data.call_id}"
            )
            return generic_hangup_response()
        ...
        # the matched config simultaneously identifies the org â€” we never
...
async def handle_inbound_telephony(
    ...
    try:
        ...
        if event_name in ("hangup", "completed", "rejected", "canceled") or call_status in ("completed", "hangup", "canceled", "failed", "busy"):
            logger.info(
                f"[legacy /inbound/{{workflow_id}}] ignoring terminal event='{event_name}' status='{call_status}' for call_id={normalized_data.call_id}"
            )
            return generic_hangup_response()
...
# (heavy) route module â€” which transitively depends on status_processor,
# campaign helpers, etc. â€” is only loaded when the HTTP layer is actually
