...
FISH_PROVIDER_MODEL_CONFIG = provider_model_config(
    "Fish Audio",
    description=(
        "Fish Audio real-time WebSocket TTS with lifelike voice models. "
        "The voice is a Fish Audio voice model ID (reference_id), e.g. a cloned "
        "voice or a public voice."
    ),
    provider_docs_url="https://docs.fish.audio/",
)

FISH_TTS_MODELS = ["s2.1-pro", "s2.1-pro-free", "s2-pro", "s1"]
RUMIK_PROVIDER_MODEL_CONFIG = provider_model_config(
    "Rumik AI",
    description=(
        "Rumik Silk text-to-speech. mulberry is the fast, description-driven "
        "voice (natural-language voice description, optional named speaker). "
        "24 kHz PCM output."
    ),
...
class FishAudioTTSConfiguration(BaseTTSConfiguration):
    ...
    model: str = Field(
        default="s2.1-pro",
        description=(
            "Fish Audio TTS model. s2.1-pro is the recommended production model; "
            "s2.1-pro-free is the free tier for testing."
        ),
        json_schema_extra={
            "examples": FISH_TTS_MODELS,
            "allow_custom_input": True,
        },
    )
    voice: str = Field(
        default="",
        description=(
            "Fish Audio voice model ID (reference_id) — a cloned voice or a "
            "public voice such as Sarah (933563129e564b19a115bedd57b7406a)."
        ),
        json_schema_extra={"allow_custom_input": True},
    )
    language: str = Field(
        default="en",
        description=(
            "ISO 639-1 language code. Fish Audio auto-detects language from "
    ...
    speed: float = Field(
        ...
        ge=0.5,
        le=2.0,
        description="Speech speed multiplier (prosody speed, 0.5 to 2.0).",
    )
...
class RumikTTSConfiguration(BaseTTSConfiguration):
    ...
    model: str = Field(
        default="mulberry",
        description=(
            "Rumik Silk TTS model. mulberry is the fast, description-driven "
            "voice (Hindi in Devanagari + English in Latin, code-mixed); muga "
            "is the expressive voice steered by tone tags."
        ),
        json_schema_extra={
            "examples": RUMIK_TTS_MODELS,
            "allow_custom_input": True,
        },
    )
    voice: str = Field(
        default="",
        description=(
            "Optional named speaker preset to pin (e.g. 'siya'). Leave empty to "
            "build the voice purely from the description."
        ),
        json_schema_extra={
            "examples": RUMIK_TTS_VOICES,
            "allow_custom_input": True,
        },
    )
    description: str = Field(
        ...
        description=(
            "Natural-language voice description. Required for mulberry: it drives "
            "gender, age, timbre, accent, and pacing."
        ),
        json_schema_extra={"allow_custom_input": True},
    )
    language: str = Field(
        default="hi",
        description=(
            "Language hint (informational). Mulberry auto-detects from the text "
            "and steers the accent from the description; Hindi is written in "
            "Devanagari, English in Latin (code-mixed)."
        ),
        json_schema_extra={"allow_custom_input": True},
    )
    speed: float = Field(
        ...
        ge=0.5,
        le=2.0,
        description="Accepted for compatibility; mulberry has no speed parameter.",
    )
    base_url: str = Field(
        default="https://silk-api.rumik.ai",
        description="Rumik Silk API gateway base URL.",
        json_schema_extra={"allow_custom_input": True},
    )
