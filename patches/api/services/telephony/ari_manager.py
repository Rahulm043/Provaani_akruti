...
class ARIConnection:
    ...
    async def _handle_dtmf_digit(self, workflow_run_id: str, digit: str):
        """Handle incoming DTMF digit from caller, buffer in Redis, and publish completed numbers."""
        if not digit:
            return
        try:
            r = await self._get_redis()
            buf_key = f"dtmf:buf:{workflow_run_id}"

            # Asterisk '*' clears the buffer
            if digit == "*":
                await r.delete(buf_key)
                logger.info(
                    f"[ARI org={self.organization_id}] DTMF buffer cleared for run {workflow_run_id}"
                )
                return

            current_buf = await r.get(buf_key) or ""
            new_buf = current_buf + digit
            await r.set(buf_key, new_buf, ex=300)
            logger.info(
                f"[ARI org={self.organization_id}] DTMF buffer for run {workflow_run_id}: {new_buf}"
            )

            numeric_digits = "".join([c for c in new_buf if c.isdigit()])

            should_dispatch = False
            extracted_number = ""

            if digit == "#" and len(numeric_digits) >= 10:
                should_dispatch = True
                extracted_number = numeric_digits[-10:]
            elif len(numeric_digits) >= 10:
                should_dispatch = True
                extracted_number = numeric_digits[-10:]

            if should_dispatch:
                logger.info(
                    f"[ARI org={self.organization_id}] DTMF 10-digit number captured for run {workflow_run_id}: {extracted_number}"
                )
                await r.delete(buf_key)
                event_payload = json.dumps(
                    {
                        "event": "dtmf_complete",
                        "digits": extracted_number,
                        "workflow_run_id": workflow_run_id,
                    }
                )
                channel_name = f"dtmf:channel:{workflow_run_id}"
                await r.publish(channel_name, event_payload)
        except Exception as e:
            logger.error(
                f"[ARI org={self.organization_id}] Error handling DTMF digit for run {workflow_run_id}: {e}"
            )
    ...
    async def _handle_event(self, raw_data: str):
        ...
        if event_type == "StasisStart":
            ...
            logger.info(
                f"[ARI org={self.organization_id}] DTMF received: "
                f"channel={channel_id}, digit={digit}"
            )
            workflow_run_id = await self._get_channel_run(channel_id)
            if workflow_run_id and digit:
                asyncio.create_task(
                    self._handle_dtmf_digit(str(workflow_run_id), digit)
                )
    ...
    async def _handle_stasis_start(
        ...
        try:
            ...
            if created_id != ext_channel_id:
                logger.info(
                    f"[ARI org={self.organization_id}] Asterisk assigned channel id "
                    f"{created_id} (requested {ext_channel_id}). Rebinding state..."
                )
                await self._pop_pending_bridge(ext_channel_id)
                await self._delete_channel_run(ext_channel_id)
                await self._delete_ext_channel(ext_channel_id)

                ext_channel_id = created_id
                await self._mark_ext_channel(ext_channel_id)
                await self._set_channel_run(ext_channel_id, workflow_run_id)
                await self._set_pending_bridge(ext_channel_id, channel_id, workflow_run_id)
                await db_client.update_workflow_run(
                ...
                # For chan_websocket, externalMedia is active upon return from the REST call.
                # Complete the bridge immediately.
                pending = await self._pop_pending_bridge(ext_channel_id)
                if pending:
                    asyncio.create_task(
                        self._complete_bridge_after_ext_ready(ext_channel_id, pending)
                    )
                return
