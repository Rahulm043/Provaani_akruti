...
class CampaignCallDispatcher:
    ...
    async def process_batch(self, campaign_id: int, batch_size: int = 10) -> int:
        ...
        for i, queued_run in enumerate(queued_runs):
            try:
                # Apply rate limiting scaled to the campaign's max concurrency
                # so that concurrent calls can be initiated together.
                concurrency = (
                    campaign.orchestrator_metadata.get("max_concurrency")
                    if campaign.orchestrator_metadata
                    else 1
                )
                effective_rate = campaign.rate_limit_per_second * max(
                    concurrency or 1, 1
                )
                await self.apply_rate_limit(campaign.organization_id, effective_rate)
