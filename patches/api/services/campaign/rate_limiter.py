...
class RateLimiter:
    ...
    async def try_acquire_concurrent_slot(
        ...
        Returns an object with slot_id and active_count if successful, None if limit reached.
        """
        redis_client = await self._get_redis()
    ...
    async def try_acquire_concurrent_slot_details(
        ...
        """Alias for try_acquire_concurrent_slot (new API method name)."""
        return await self.try_acquire_concurrent_slot(
            organization_id,
            max_concurrent,
            scope_key=scope_key,
    ...
    async def release_concurrent_slot(
        self, organization_id: int, slot_id: str, scope_key: str | None = None
    ) -> bool:
        """
    ...
    async def store_workflow_slot_mapping(
        ...
        try:
            data = {"org_id": organization_id, "slot_id": slot_id}
            if scope_key:
                data["scope_key"] = scope_key
            await redis_client.hset(mapping_key, mapping=data)
    ...
    async def store_workflow_slot_mapping_if_absent(
        ...
        """Store mapping only if not already bound. Returns True if stored."""
        redis_client = await self._get_redis()
        mapping_key = f"workflow_slot_mapping:{workflow_run_id}"

        try:
            exists = await redis_client.exists(mapping_key)
            if exists:
                return False
            return await self.store_workflow_slot_mapping(
                workflow_run_id, organization_id, slot_id, scope_key=scope_key
            )
        except Exception as e:
    ...
    async def get_workflow_slot_mapping(
        ...
        Returns (organization_id, slot_id, scope_key) tuple or None.
        """
        redis_client = await self._get_redis()
        mapping_key = f"workflow_slot_mapping:{workflow_run_id}"
        ...
        try:
            ...
            if mapping and "org_id" in mapping and "slot_id" in mapping:
                ...
                return (int(mapping["org_id"]), mapping["slot_id"], scope_key)
            return None
        except Exception as e:
    ...
    async def acquire_from_number(
        ...
        -- PATCHED: Allow concurrent use of the same phone number.
        -- Plivo supports multiple concurrent calls from a single number.
        -- Instead of checking availability, return any active number from the pool.
        local all = redis.call('ZRANGEBYSCORE', key, '-inf', '+inf')
        if #all == 0 then
            return nil
        end

        local idx = math.random(#all)
        return all[idx]
        """
