...
async def _run_pipeline_impl(
    ...
    logger.info(
        f"[run {workflow_run_id}] resolved user_turn_stop_timeout={user_turn_stop_timeout}"
    )
        ...
        async def on_latency_measured(observer, latency_seconds):
            ...
            if task.turn_trace_observer:
                try:
                    ...
                    if trace_id:
                        trace_url = get_trace_url(trace_id, organization_id)
                except Exception as e:
                    logger.debug(f"Failed to build trace URL: {e}")
            logger.info(
                "[latency] run={} latency_seconds={:.3f} node={} trace={}",
                workflow_run_id,
                latency_seconds,
                in_memory_logs_buffer.current_node_name,
                trace_url,
            )
            message = {
