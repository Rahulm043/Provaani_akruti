...
def create_tts_service(
    ...
    if user_config.tts.provider == ServiceProviders.DEEPGRAM.value:
        ...
        settings_kwargs = FishAudioTTSSettings(
            model=user_config.tts.model,
            voice=user_config.tts.voice,
            latency="balanced",
        )
        ...
        # Rumik Silk TTS ("silk mulberry") streams 24 kHz mono PCM over WebSocket.
        # The pipeline's audio transport resamples to the telephony rate, so we
        # always request the service's native sample rate here.
        base_url = (
            getattr(user_config.tts, "base_url", None) or "https://silk-api.rumik.ai"
        ).rstrip("/")
        _validate_runtime_service_url(base_url, "base_url")
        settings_kwargs: dict = {
            "model": user_config.tts.model,
            "description": getattr(user_config.tts, "description", None)
            or RUMIK_DEFAULT_DESCRIPTION,
        }
        ...
        return RumikTTSService(
            ...
            sample_rate=24000,
            settings=RumikTTSService.Settings(**settings_kwargs),
            text_filters=[xml_function_tag_filter],
            skip_aggregator_types=["recording_router", "recording"],
            silence_time_s=1.0,
        )
